...
public class TaskGroupService
{
	...
	(
			...
			content      = @Content(mediaType = MediaType.APPLICATION_JSON, schema = @Schema(implementation = TaskGroupDTO.class))
	)
	@GET
	public Response findAll()
	{
		try
		{
			...
			all.forEach(taskGroup -> result.add(taskGroup.toDTO(context)));
		...
		try
		{
			return
					service
							...
							(
									taskGroup ->
									Response.ok(taskGroup.toDTO(new ReferenceCycleTracking()))
									.build()
							)
							.orElseGet(() -> Response.status(Response.Status.NOT_FOUND).entity(String.format(MSG_TASK_GROUP_NOT_FOUND, id))
							.build());
		}
		catch (Exception e)
		{
	...
	)
	{
		try
		{
			...
			{
				TaskGroupDTO result = optional.get().toDTO(new ReferenceCycleTracking());
				return Response.ok(result).build();
			}
			else
			{
				return Response.status(Status.NOT_FOUND).entity(String.format(MSG_TASK_GROUP_NOT_FOUND, id)).build();
			}
		}
		catch (Exception e)
		{
	...
	(
			responseCode = "200",
			description  = "task group found",
			content      = @Content(mediaType = MediaType.APPLICATION_JSON,schema = @Schema(implementation = TaskGroupLazy.class))
	)
	@APIResponse
		(
			responseCode = "404",
		...
		try
		{
			...
			if (optional.isPresent())
			{
				...
				return Response.ok(result).build();
					...
					return Response.status(Status.NOT_FOUND).entity("company with id " + id + " not found").build();
		}
		catch (Exception e)
		{
...
//					internalServerError(e, "Failed to retrieve task groups list");
	...
	public Response create(@Valid @NonNull TaskGroupDTO dto)
	{
		...
		{
			ReferenceCycleTracking context = new ReferenceCycleTracking();
			TaskGroupJPA     entity  = service.create(dto.toEntity(context));
			return
					Response.status(Status.CREATED)
							.entity(entity.toDTO(context))
							.build();
		}
		catch (Exception e)
		{
	...
	public Response update(@Valid @NonNull TaskGroupDTO dto)
	{
		...
		{
			ReferenceCycleTracking context = new ReferenceCycleTracking();
			TaskGroupJPA     entity  = service.update(dto.toEntity(context));
			return Response.ok(entity.toDTO(context)).build();
		}
			...
			@PathParam("idGroup") @NonNull Long idGroup, @PathParam("idTask") @NonNull Long idTask)
	{
		try
		{
			...
			return Response.ok().build();
				...
				{ return Response.status(Status.NOT_FOUND).entity(String.format(MSG_TASK_GROUP_NOT_FOUND, idGroup)).build(); }
		catch (TaskNotFoundException e)
				{ return Response.status(Status.NOT_FOUND).entity(String.format(MSG_TASK_NOT_FOUND      , idTask)).build(); }
		catch (Exception e)
		{
